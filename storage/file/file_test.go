...
func TestFileStorage_GetOne(t *testing.T) {
	var (
		req   = require.New(t)
		file  = getTestFileRepo()
	...
	//t.Run("select type counter", test(&metrics[1]))
}
...
func TestFileStorage_SetOne(t *testing.T) {
	var (
		req   = require.New(t)
		file  = getTestFileRepo()
		value = float64(3702784)
	...
	t.Run("insert type gauge", test(metrics[0]))
	t.Run("insert type counter", test(metrics[1]))
}

func TestFileStorage_SetAll(t *testing.T) {
	var (
		req   = require.New(t)
		file  = getTestFileRepo()
		value = float64(3702784)
